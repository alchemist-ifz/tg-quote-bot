...
import emojiImageByBrand from './emoji-image.js';
...
function loadFont() {
  ...
  for (const fileName of files) {
    ...
    try {
      registerFont(filePath, { family: fileName.replace(/\.[^/.]+$/, '') });
    } catch (error) {
      console.error(`${filePath} not font file`);
    }
    ...
    if (avatarImageCache) {
      ...
      try {
        ...
        if (user.photo && user.photo.big_file_id) {
          userPhotoUrl = await this.telegram.getFileLink(user.photo.big_file_id).catch(console.error)
        }
        ...
        if (!userPhotoUrl) {
          ...
          if (userPhoto) {
            userPhotoUrl = await this.telegram.getFileLink(userPhoto)
        ...
        if (userPhotoUrl) {
          avatarImage = await loadImage(userPhotoUrl.toString())
        }
